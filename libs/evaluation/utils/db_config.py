...
from evaluation.utils import mysql_to_sqlite, tools
...
if not os.path.isfile(DATABASE_DIR()):
    zip_file = str(DATABASE_DIR()).replace('.db', '.zip')
    if os.path.isfile(zip_file):
        tools.unzipfiles(zip_src=zip_file,
                         dst_path=DATABASE_DIR().parent)
        os.remove(zip_file)
        print(
            "\x1b[32m",
