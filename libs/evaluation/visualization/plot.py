...
from evaluation.processing import integration, tools_ec
...
def get_style(
    ...
    :param old_scaling: bool. default False
        use old_scaling changed since 2025-01-20
    :param scale_fontsize: float,default 1
        scale all font size params by given factor
    :return: params
    ...
    scaling = 0.3937  # scaling factor from cm to inch
    if old_scaling:  # somehow before 20250120 this factor was wrong!
        print('You are using wrong scaling factor. Figure size will be larger by ~11% than set in fig_size')
        scaling = 0.4384
    ...
    if style == "singleColumn":
        ...
        raise Exception("Style ", style, ' not known! Try "singleColumn".')
    ...
    params = {
        ...
        "font.size": 20 * scaling * scale_fontsize,  # Set font size to 20pt Arial
        "axes.labelsize": 18 * scaling * scale_fontsize,  # -> axis labels
        "legend.fontsize": 16 * scaling * scale_fontsize,  # legend font size
        # linewidth
        ...
        "hatch.linewidth": 1.5 * scaling,  # edge width in points. Hatch = shape/lines/.. inside a 2D area
        # Grid
        ...
        "xtick.labelsize": 18 * scaling * scale_fontsize,  # x tick labels
        ...
        "ytick.labelsize": 18 * scaling * scale_fontsize,  # y tick labels
...
# makes this editable by user within the Notebook
manual_col_to_axis_label = {
...
def get_axis_label(col_name):
    ...
    # manual colnames
    if col_name in manual_col_to_axis_label.keys():
        return manual_col_to_axis_label[col_name]
...
def get_geo_column(name, type_in, type_out, analysis_parameter=True):
    """
    Shorthand notation to get geo_column name of another type.
    :param name: list or str
        name of the column of type_in
    :param type_in: str one of ["A_geo", "geo_label", "dm_dt_S", "j_geo", "R_u_geo"]
        type of the input column name
    :param type_out: str one of ["A_geo", "geo_label", "dm_dt_S", "j_geo", "R_u_geo"]
        type of the output column name
    :param analysis_parameter: bool, default True
        if False columns in name which have analyis_parameter=True will be ignored
    :return: list or str (depending on type of name)
    """
    if name is None:
        return geo_columns.loc[lambda row: (~row.analysis_parameter | analysis_parameter), type_out].to_list()

    name_list = [name] if type(name) == str else name
    if geo_columns.loc[:, type_in].isin(name_list).any():
        name_out = geo_columns.loc[
            lambda row: (row.loc[:, type_in].isin(name_list)
                         # include analysis_parameter only if analysis_parameter==True
                         & (~row.analysis_parameter | analysis_parameter)
                         ),
            type_out
        ].to_list()
        return name_out[0] if type(name) == str else name_out
    else:
        return None
...
def get_geo_column_label(col_name):
    ...
    if idx_find[0] == "R_u_geo":
        return (
            "$HFR_\mathrm{geo}$ / m$\Omega$ cm$_{(\mathrm{"
            + geo_columns.loc[idx_find[1], "geo_label"]
            + "})}^{-2}$"
        )
...
geo_columns = pd.DataFrame(
    [
        ...
        [
            ...
            "R_u__mOhm_cm2geo_fc_top_cell_Aideal",
            False,
        ],
        [
            ...
            "R_u__mOhm_cm2geo_fc_top_cell_Areal",
            False,
        ],
        [
            ...
            "R_u__mOhm_cm2geo_fc_top_sealing",
            False,
        ],
        [
        ...
        [
            ...
            "R_u__mOhm_cm2geo_fc_bottom_cell_Aideal",
            False,
        ],
        [
            ...
            "R_u__mOhm_cm2geo_fc_bottom_cell_Areal",
            False,
        ],
        [
            ...
            "R_u__mOhm_cm2geo_fc_bottom_sealing",
            False,
        ],
        [
            ...
            "R_u__mOhm_cm2geo_fc_bottom_PTL",
            False,
        ],
        [
            "A_geo_active_chosen__mm2",
            "active\ chosen",
            "dm_dt_S__ng_s_cm2geo_fc_active_chosen",
            "j__mA_cm2geo_active_chosen",
            "R_u__mOhm_cm2geo_active_chosen",
            True,
        ],
    ],
    columns=["A_geo", "geo_label", "dm_dt_S", "j_geo", "R_u_geo", "analysis_parameter"],
)
...
def add_empty_subplot(fig, gs_pos):
    """
    Add a subplot axes without axis, spines, ticks,
    :param fig: matplotlib.figure
        figure in which to place
    :param gs_pos: position of the subplot as used in fig.add_subplot()
    :return: ax
    """
...
def log_axis_xticklabel(ax,
                        one_label_per_order=True,
                        style='decimal',
                        strip_decimal_0=True,
                        ):
    """
    Modify x-ticklabels for log axis
    :param ax:
    :param one_label_per_order: bool
        if true: set one axis x-ticklabel per order of magnitude
    :param style: str one of
        'decimal': 0.1, 0.01, 0.001,...
        'scientific': 10^{-1}, 10^{-2}, 10^{-3}, ..
    :param strip_decimal_0: bool
        only applies if style='decimal',
        True: 0.1, 0.01, 0.001
        False: 0.100, 0.010, 0.001
    :return:
    """
    """
    ...
    :return:
    """
    # ax5 x ticklabels -- ensuring one label for each order of magnitude
    x_min, x_max = ax.get_xlim()
    # print('jo', x_min, x_max)

    # Calculate the range of orders of magnitude
    min_order = np.floor(np.log10(x_min)) + 1
    max_order = np.ceil(np.log10(x_max)) - 1

    if one_label_per_order:
        # Create ticks based on the order of magnitude
        ticks = np.logspace(min_order, max_order, num=int(max_order - min_order) + 1)
        # print(ticks)

        # Set the x-ticks to one label for each order of magnitude
        ax.set_xticks(ticks)
    else:
        ticks = ax.get_xticks()
    if style == 'decimal':
        max_decimal_digits = int(- (min_order))
        # print(ticks, max_decimal_digits, min_order, x_min)
        max_decimal_digits = max_decimal_digits if max_decimal_digits >= 0 else 0
        # print(ticks, max_decimal_digits)
        ticklabels = [f"{tick:.{max_decimal_digits}f}" for tick in ticks]
        if strip_decimal_0:
            ticklabels = [ticklabel.rstrip('0').rstrip('.') for ticklabel in ticklabels]
        ax.set_xticklabels([ticklabel for ticklabel in ticklabels])
    elif style == 'scientific':
        print('scientific should be default, if not to be developed')
    else:
        print("\x1b[33m", "Style must be one of ['decimal', 'scientific']", "\x1b[0m")


def log_axis_yticklabel(ax,
                        one_label_per_order=True,
                        style='decimal',
                        strip_decimal_0=True,
                        ):
    """
    Modify y-ticklabels for log axis
    :param ax:
    :param one_label_per_order: bool
        if true: set one axis y-ticklabel per order of magnitude
    :param style: str one of
        'decimal': 0.1, 0.01, 0.001,...
        'scientific': 10^{-1}, 10^{-2}, 10^{-3}, ..
    :param strip_decimal_0: bool
    ...
    if style == 'decimal':
        ...
        if strip_decimal_0:
            ticklabels = [ticklabel.rstrip('0').rstrip('.') for ticklabel in ticklabels]
        ax.set_yticklabels([ticklabel for ticklabel in ticklabels])
    elif style == 'scientific':
        print('scientific should be default, if not to be developed')
    else:
        print("\x1b[33m", "Style must be one of ['decimal', 'scientific']", "\x1b[0m")
...
def synchronize_timestamps(
    ...
    :param timestamp_col_ec: str, optional, default 'Timestamp',
        name of the timestamp column considered for the synchronization for the EC data
    :param timestamp_col_icpms: str, optional, default 't_delaycorrected__timestamp_sfc_pc',
        name of the timestamp column considered for the synchronization for the ICPMS data
...
def synchronize_timestamps_multiple(
    ...
    if overlay_index_cols is not None and overlay_index_cols != []:
...
def get_j__mA_mg(
    ...
    """
    print("\x1b[33m", "plot.get_j__mA_mg is deprecated. "
                      "Please use the method: tools_ec.gravimetric_current.", "\x1b[0m")
    return tools_ec.gravimetric_current(exp_ec,
                                        data_ec,
                                        j_geo_cols=j_geo_col,)


def aggregate_list_unique(df,
                          on,
                          cols_first_value=None,
                          print_report=True,
                          aggregate_prev_index_cols=True,
                          ):
    """
    Aggregate a DataFrame and writes all unique values into a list.
    If there is only one unique value in a group this is set.
    :param df: pd.DataFrame
        dataframe to be aggregated
    :param on: str or list of str
        columns to perform the aggregation on, as used in pd.DataFrame.groupby()
    :param cols_first_value: list of str, optional default None
        name of columns from which the first value should be selected even if different values are across the rows
    :param print_report: bool, default True
        whether to print report (colored dataframe) or not
    :param aggregate_prev_index_cols: bool, default True
        whether to aggregate previous index columns. This will reset_index before aggregation.
    :return: pd.Dataframe
        aggregated DataFrame
    """
    if cols_first_value is None:
        cols_first_value = []

    if aggregate_prev_index_cols:
        df = df.reset_index()

    # dictionary: keys columns which are aggregated, values: list of aggregations performed on these columns
    report_style = {col: [] for col in
                    df.columns}

    def aggregate_group_list_unique(col):
        try:
            if col.name in cols_first_value or len(col.drop_duplicates()) <= 1:
                report_style[col.name] += ['u']
                return col.iloc[0]
            else:
                report_style[col.name] += ['n']
                return list(col.drop_duplicates())

        except Exception as e:
            warnings.warn('Problems aggregating column "%s" of data type %s' % (col.name, type(col.iloc[0])))
            display(col)
            report_style[col.name] += ['f']
            return 'failed aggregation'

    result_df = df.groupby(on).agg(aggregate_group_list_unique)

    def result_style(col):
        return ['background-color: darkgreen' if report == 'u' else
                'background-color: darkorange' if report == 'n' else
                'background-color: darkred' if report == 'f' else
                ''
                for report in report_style[col.name]]

    if print_report:
        found_types = {x for l in report_style.values() for x in l}  # set of found types
        if 'u' in found_types:
            print("\x1b[32m", 'green: A single value per group.', "\x1b[0m")
        if 'n' in found_types:
            print("\x1b[33m", 'orange: Different values within the group --> stored as list.', "\x1b[0m")
        if 'f' in found_types:
            print("\x1b[31m", 'red: Failed aggregation please report to admin.', "\x1b[0m")
        display(result_df.style.apply(result_style, axis=0).set_table_styles([
            {'selector': 'td', 'props': [('overflow', 'hidden'),
                                         ('white-space', 'nowrap'),
...
def aggregate_unique_or_list(aggregate_group, cols_first_value=None):
    """
    Depracated please use: aggregate_list_unique()
    Aggregate a DataFrame group (pd.DataFrame.groupby(on).apply(aggregate_unique_or_list))
    for each column to unique value or list of different values
    For each column check whether non-unique values
    - if there are different values, return a list of those
    - if all the same values just return this value
    :param cols_first_value: list of str, optional default None
        name of columns from which the first value should be selected even if different values are across the rows
    :param aggregate_group: pd.DataFrame
        to be aggregated DataFrame
    :return: pd.Dataframe
        aggregated DataFrame
    """
    warnings.warn('This function is deprecated and will be removed in future versions. Please use aggregate_list_unique().')
    if cols_first_value is None:
        cols_first_value = []

    def aggregate_diff_cols_to_list(col):
        """
        Aggregate pd.Series to unique value or list of different values
        :param col: pd.Series
            series of the columnt to be aggregated
        :return: value or pd.Series
            aggregated column
        """
        def aggregate_list():
            pd.Series({col.index[0]: list(col.tolist())})
        try:
            if col.name in cols_first_value:
                return col.iloc[0]
            elif type(col.iloc[0]) == list:
                print(
                    "\x1b[33m"
                    + 'Aggregation warning: Column %s of type list cannot be aggregated, lists are just appended'
                      % col.name
                    + "\x1b[0m"
                )
                ...
                print("\x1b[33m"
                      + 'Aggregation warning: Different values found in column: %s %s' % (col.name, col.tolist())
                      + "\x1b[0m"
                      )
    ...
    return aggregate_group.apply(
        lambda col: aggregate_diff_cols_to_list(col)
    )
...
def average(
    ...
    :param cols_first_value: list of str, optional default None
        name of columns from which the first value should be selected even if different values are across the
        experiments
    ...
    #exp_avg = exp.groupby(on).apply(lambda x: aggregate_unique_or_list(x, cols_first_value=cols_first_value))
    # new function which is working as expected:
    exp_avg = aggregate_list_unique(exp, on=on, cols_first_value=cols_first_value)
    ...
    if average_cols is None:
        ...
        ).columns.tolist()  # .columns[data.dtypes == 'float64']
    ...
    # Problem if col_std already in columns (example_ I__A, I__A_std)
    avg_cols_drop = []
    for avg_col in average_cols:
        #print(avg_col)
        if any([avg_col+'_std' in average_cols,
                #avg_col+'_max' in average_cols,
                #avg_col+'_max' in average_cols,
                #avg_col+'_count' in average_cols,
               ]):
            print(avg_col+'_std', ' already in data columns. Will drop the column calculate the std from standard deviation of averaging. Be sure that the previous std is not larger.')
            avg_cols_drop += [avg_col+'_std']
            average_cols.remove(avg_col+'_std')
    data = data.drop(columns=avg_cols_drop)
    ...
    # Store number of averaged experiments in experiment dataframe
    exp_avg.loc[:, 'avg_counts'] = data_avg.groupby(on).avg_counts.unique()
...
class curve_fit_odr_style:
    ...
    def __init__(self, f, xdata, ydata, **kwargs):
        ...
        if 'verbose' in kwargs.keys():
            if kwargs['verbose'] == 2:
                # Overparametrized?
                print(f'Overparametrized? This value should be "small": {np.linalg.cond(pcov)}')
...
def create_fit_label(
    ...
    return dict(
        ...
        params=params,
        params_selection=params_selection,
        rsquared=rsquared,
...
class datasetAccessor:
    ...
    def __init__(self, pandas_obj):
        ...
        if not hasattr(self._obj, "export_name"):
            ...
            # Problem! Setting an attribute to a DataFrame
            # This can break selection of columns if column name export_name
            # Also, the attribute is removed when variable is resetted (joined new columns onto it all at once)
            #   check dataset.fit() --> had to workaround the name to resetted DataFrame
    ...
    def return_dataset(self, return_data=False):
        """
        Return DataFrame
        ...
        if return_data:
            if not hasattr(self, 'data'):
                warnings.warn('No data attribute defined on caller. Return empty DataFrame')
                self.data = pd.DataFrame([])
            return self._obj, self.data
        return self._obj

    def display(self, *args, only_different_cols=False):
        """
        ...
        :param change display options as defined in pd.context_options. Define as: ``pat, val, [(pat, val), ...]``.
        :return: None
        """
        dataFrame = self._obj
        ...
        args_changed_default = list(args) \
                               + (["display.max_rows", None] if "display.max_rows" not in args else [])\
                               + (["display.max_columns", None] if "display.max_columns" not in args else [])

        with pd.option_context(*args_changed_default):  # more options can be specified also
            try:
                display(dataFrame)  # works only in Jupyter Notebook/Lab environment
            except NameError:
                print(dataFrame)

    def _colormap(self, cmap_name, rowindexer=slice(None), cmap_min=0, cmap_max=1, cmap_group=None):
        """
        ...
        :param cmap_max: float, cmap_min<cmap_max<1
            cut right side of colormap
        :param cmap_group: str, list of str or None
            defining groups of experiments by columns or index columns.
            Each group of experiments will get an individual color (instead of each individual experiment)
        :return: pd.Series with the color value for each row
        """
        self_obj_subselected_rowindexer = self._obj.loc[rowindexer, :]

        if cmap_group is None:
            count_rows = self_obj_subselected_rowindexer.shape[0]
            return pd.Series(
                data=get_colormap(
                    cmap_name, count_rows, cmap_min=cmap_min, cmap_max=cmap_max
                ),
                index=self_obj_subselected_rowindexer.index,
                dtype="object",
            )
        else:
            cmap_group = [cmap_group] if type(cmap_group) == str else cmap_group
            count_rows = len(self_obj_subselected_rowindexer.reset_index().loc[:, cmap_group].drop_duplicates().index)
            df_color = self_obj_subselected_rowindexer.groupby(cmap_group).first()
            df_color = df_color.assign(color=get_colormap(cmap_name, count_rows, cmap_min=cmap_min, cmap_max=cmap_max))

            # set(cmap_group+self._obj.index.names) --> avoid duplicate columns if cmap_group is index column
            return self_obj_subselected_rowindexer.reset_index()\
                            .loc[:, list(set(cmap_group+self_obj_subselected_rowindexer.index.names))]\
                            .join(df_color.color, on=cmap_group)\
                            .set_index(self_obj_subselected_rowindexer.index.names)\
                            .color
    ...
    def add_column(
        ...
        **kwargs,
        ...
        :param values: list of Any or Any or pd.Series
            if list: must be of same length as rowindexers,
            if Any: all rows will have this value
            if pd.Series: values will be set according to index in pd.Series,
                index columns must be contained index columns of caller dataframe
                exception: for name='color', values can have any index columns
                    from all columns existing in caller dataframe (values dataframe will be joined)
                    workaround to make translation of colormap to color value possible
        ...
        :param kwargs: bool, default False
            get additional prints to understand error messages
        :param kwargs:
            cmap_min, cmap_max for removing edges of selected colormap
            and cmap_group to get individual color for each group instead of each experiment.
        :return:
        """
        ...
        if rowindexer_first_of_group is not None:
            ...
            rowindexer = self._obj.index.isin(self._obj.reset_index()
                                                    .groupby(rowindexer_first_of_group, dropna=False)
                                                    .first()
                                                    .reset_index()
                                                    .set_index(self._obj.index.names)
                                                    .index
                                                    )
            if len(rowindexer) == len(values) and rowindexer.sum() != len(values) and type(values) != pd.Series:
                # if values is not given as pd.Series with their specific index,
                # also the number of values must be reduced according to first_of_group
                # select from values where rowindexer (is a boolean list) is True, works not with list but with np.array
                values = np.array(values)[rowindexer].tolist()

            rowindexers = [rowindexer, slice(None)]
            values = [values, None]
        # print(rowindexers)
        # print(values)
        if values is None:
        ...
        if rowindexers is None:
            # Exception for color --> Make type(values) = pd.Series possible for colormap transformation
            # This is just a workaround to get intuitive behavior for user, here rowindexers are defined
            # Usually when giving pd.Series as value joining is performed within pandas
            # There might be more efficient routine using pd.DataFrame.join(values, on=values.index.names)
            if name == "color" and type(values) == pd.Series:
                # if name in self._obj.columns: - default behavior overwriting previous column
                #    print('Column ', name, ' already exists in given DataFrame and will be overwritten.')
                #    self._obj.drop(columns=name, inplace=True)
                # join given values to caller'S DataFrame
                df_joined_values = self._obj.join(values, on=values.index.names).loc[:, values.name]
                # Derive values and rowindexers, take the order of appearance in df_joined_values
                values = df_joined_values.dropna().unique().tolist()  # only values with index existing in self.obj
                rowindexers = [df_joined_values == val for val in values]

                # old approach - but problem with order of values and rowindexers
                #rowindexers = [(self._obj.reset_index().loc[:, values.index.names] == col_val).all(axis=1).tolist()
                #               for col_val in self._obj.reset_index().loc[:, values.index.names].drop_duplicates().values
                #               if (tuple(col_val) if len(col_val) > 1 else col_val[0])  # differ single - multiindex
                #                  in values.index]  # filter out rowindexers not specified in values
                #values = [val for idx, val in values.items()
                #              if idx in self._obj.reset_index().set_index(values.index.names).index]
                # filter out values not considered in given self.obj
                # = values.tolist() does not consider rows given in values but not used in self.obj
                if debug:
                    print(rowindexers)
                    print(values)
                #print(len(rowindexers), len(values))
            else:
        ...
        for (rowindexer, value) in zip(rowindexers[::-1], values[::-1]):
            ...
            if type(value) == list and type(rowindexer) == list:
                if len(value) != len(rowindexer):
                    warnings.warn('Different length in rowindexer (%s) and value (%s) may lead to errors.'
                                  % (len(value), len(rowindexer)))
            #print(rowindexer, value)
            if debug:
                print(rowindexer)
                print(value)
    ...
    def _extend_kwargs_from_dataframe(
        row, index, plt_kwargs, possible_kwarg_params_name, ignore_cols=None,
    ):
        """
        ...
        :param ignore_cols: list or None
            list of columns to be ignored even if existent
        :return: dict
        ...
        for param_name in row.keys():
            if (
                ...
                and param_name not in ignore_cols
            ):
    ...
    def plot(
        ...
        ax_plot_object_name=None,
        plot_type="plot",
        plot_nans=False,
        rowindexer=None, # idea to select experiments here and only loop over these
        plt_kwargs_extend_ignore_cols=None,
        add_data_to_export_object=False,
        **plt_kwargs,
    ):
        """
        ...
        :param ax_plot_object_name: str or None, default None
            name of the column in self in which the matplotlib object is stored.
            "ax_plot_object_" + value is the column name-
            if None it will be added to the list stored in column ax_plot_objects
        ...
        :param plot_nans: bool, default False
            if True: checks whether for an index only nans are found in x_col or y_col.
                If yes:
                    throws warning: "only NaNs found in data table. Will be skipped and not plotted.
                                       Label: S_number_sum will not be added to legend."
                   and avoid addition of label
               If false: no checks, previous behaviour
        :param rowindexer: rowindexer as interpreted by pd.DataFrame().loc[rowindexer] or None
            add a plot only to a subselection of caller
        :param plt_kwargs_extend_ignore_cols: list of str or None
            list of columns to be ignored even if existent for extending plt_kwargs from dataframe columns
        :param add_data_to_export_object: bool, default False
            controls whether dataset and items are added to export_object
        :param plt_kwargs: any additional keyword arguments of matplotlib.pyplot.plot
        :return:
        """

        if plt_kwargs_extend_ignore_cols is None:
            plt_kwargs_extend_ignore_cols = []
        if len(self._obj.index) == 0:
            return self
        ...
        if rowindexer is None:
            rowindexer = slice(None)
        ...
        for i, (index, row) in enumerate(self._obj.loc[rowindexer].iterrows()):
            plt_kwargs_extended = self._extend_kwargs_from_dataframe(
                row, index, plt_kwargs, possible_kwarg_params_name, ignore_cols=plt_kwargs_extend_ignore_cols
            )
            print(
            ...
            ax = row[ax_col]
            # skip axes with nan
            if ax is None or self._obj.loc[[index, ], ax_col].isna().iloc[0]:
                continue
            ...
            if plot_data is not None:
                if index not in plot_data.index:
                    warnings.warn(
                        f"index: {str(index)} does not exist in data table. Will be skipped and not plotted. "
                            "Label " + (plt_kwargs_extended["label"]
                                            if "label" in plt_kwargs_extended.keys()
                                            else " no label given"
                                        ) + " will not be added to legend."
                    )
                    # reset label
                ...
                # index exists but only nans in data
                elif not plot_nans:
                    # need to differentiate plot_data.loc[index] being series (.shape=1) or dataframe (.shape=2, >2?)
                    x_col_test = [x_col] if len(plot_data.loc[index].shape) == 1 else x_col
                    y_col_test = [y_col] if len(plot_data.loc[index].shape) == 1 else y_col
                    if (plot_data.loc[index, x_col_test].isna().all()
                            or plot_data.loc[index, y_col_test].isna().all()):
                        warnings.warn(
                            f"For index: {str(index)} only NaNs found in data table. Will be skipped and not plotted. "
                            "Label " + (plt_kwargs_extended["label"]
                                            if "label" in plt_kwargs_extended.keys()
                                            else " no label given"
                                        ) + " will not be added to legend."
            ...
            if plot_type == "plot":
                ...
                if 'color' in plt_kwargs_extended.keys() and 'c' in plt_kwargs_extended.keys():
                    warnings.warn('For scatter plots either use c or color. c is used, color is ignored')
                    del plt_kwargs_extended['color']
                if empty_plot:
            ...
            # Nan should be handled earlier when initiating
            #if self._obj.loc[index, "ax_plot_objects"].isna().all():
            #    self._obj.at[index, "ax_plot_objects"] = []
            #    #print("error for ax_plot_objects  in", self._obj.loc[index, :])
            if ax_plot_object_name is None:
                self._obj.loc[index, "ax_plot_objects"].append([plot])
            else:
                col_ax_plot_objects_name = "ax_plot_object_"+str(ax_plot_object_name)
                if col_ax_plot_objects_name in self._obj.columns:
                    if self._obj.loc[index, [col_ax_plot_objects_name, ]].notna().any():
                        warnings.warn(f'Already created a plot in column {col_ax_plot_objects_name}'
                                      f' for index {str(index)}. '
                                      f'It will be overwritten use a different ax_plot_object_name to avoid.')
                self._obj.loc[index, col_ax_plot_objects_name] = plot
            if add_data_to_export_object:
    ...
    def fit(
        ...
        ax=None,
        ax_resplot=None,
        ax_resplot_relative=False,
        label=None,
        label_fit=True,
        label_fit_overwrite=False,
        label_fit_style=None,
        label_restore_prev=True,
        exp_col_label="",
        display_fit=True,
        ...
        **plt_kwargs,
        ...
        :param ifixb: list of int
            ...
            Directly available for method='scipy.odr'
            For method='scipy.optimize.curve_fit': bounds will be set as np.inf or the given parameter in beta0.
        :param curve_fit_bounds: 2-tuple of array_like or Bounds, optional
            Lower and upper bounds on parameters. Defaults to no bounds. There are two ways to specify the bounds:
            - Instance of Bounds class.
            - 2-tuple of array_like: Each element of the tuple must be either an array with the length equal to the
                number of parameters, or a scalar (in which case the bound is taken to be the same for all parameters).
                Use np.inf with an appropriate sign to disable bounds on all or some parameters.
            Only available for method='scipy.optimize.curve_fit'
        :param curve_fit_kwargs: dict()
            for example:
                - method: str one of {‘lm’, ‘trf’, ‘dogbox’}, optional
                    Method to use for optimization. See least_squares for more details. Default is ‘lm’ for unconstrained
                    problems and ‘trf’ if bounds are provided. The method ‘lm’ won’t work when the number of observations is
                    less than the number of variables, use ‘trf’ or ‘dogbox’ in this case.
                    Only available for method='scipy.optimize.curve_fit'
        ...
        :param ax_resplot: Matplotlib.Axes object or str
            Axes object for the residuals plot
            or if str: column name in the caller, which refers to an Axes Object for each dataset.
        :param ax_resplot_relative: Bool, default False
            True: plot residuals in percent of data value
            False: plot absolute residuals
        :param label: str
        ...
        :param label_restore_prev: bool, default True
            If true:
                callers column 'label' is restored to the value from before.
                The fit label is stored in column 'label_fit'.
        ...
        y_col_fitted_res = y_col_fitted + '_res'
        y_col_fitted_res_percent = y_col_fitted_res + '_percent'
        print("FitResidualsData is stored in column: ", y_col_fitted_res)
        ...
        for i, (index, row) in enumerate(self._obj.iterrows()):
            if data is not None and index not in data.index:
                warnings.warn(
                    "index: "
                    + str(index)
                    + " does not exist in data table. Will be skipped and not fitted"
                )
                continue
            if self.data.loc[index, x_col].isna().all():
                warnings.warn(
                    "For index: "
                    + str(index)
                    + ", column: "
                    + str(x_col)
                    + " has only NaNs. Will be skipped and not fitted"
                )
                continue
            if self.data.loc[index, y_col].isna().all():
                warnings.warn(
                    "For index: "
                    + str(index)
                    + ", column: "
            ...
            if method == "scipy.optimize.curve_fit":
                ...
                if curve_fit_bounds is None and ifixb is not None and beta0 is not None:
                    curve_fit_bounds = ([val*0.99999999999 if fix == 0 else -np.inf for val, fix in zip(beta0, ifixb)],
                              [val if fix == 0 else np.inf for val, fix in zip(beta0, ifixb)])
                    print('Calculated parameter bounds: \n', curve_fit_bounds) if debug else ''
                ...
                fit_results = curve_fit_odr_style(
                    ...
                    sigma=sigma,
                    absolute_sigma=True,
                    bounds=curve_fit_bounds if curve_fit_bounds is not None else (-np.inf, np.inf),
                    #method=curve_fit_method,
                    **curve_fit_kwargs,
                )
                ...
                if curve_fit_bounds is not None:
                    warnings.warn(
                        "curve_fit_bounds only implemented for method=scipy.optimize.curve_fit. "
            ...
            for param_label, value, stdev in zip(
                ...
                self._obj.loc[index, exp_col_label + fit_label["model"] + "_" + param_label] = value
                self._obj.loc[index, exp_col_label + fit_label["model"] + "_" + param_label + "_sd"] = stdev
                if debug:
                    print(f"{param_label} value: {value}, stdev: {stdev}")
            # self._obj.loc[:, fit_label['model'] + '_Rsquare'] = fit_results.res_var
            self._obj.loc[index, exp_col_label + fit_label["model"] + "_ResVar"] = res_var
            self._obj.loc[index, exp_col_label + fit_label["model"] + "_Rsquared"] = rsquared
            self._obj.loc[index, exp_col_label + "fit_method"] = method  # 'scipy.odr'


            # depending on model function: type of returned values might vary, pd.Series or np.array
            y_col_fitted_values = model(
                fit_results.beta, self.data.loc[index, x_col].astype(float)
            )
            if type(y_col_fitted_values) == pd.core.series.Series:
                self.data.loc[index, y_col_fitted] = y_col_fitted_values.values
            else:
                self.data.loc[index, y_col_fitted] = y_col_fitted_values
            self.data.loc[index, y_col_fitted_res] = self.data.loc[index, y_col] - self.data.loc[index, y_col_fitted]
            self.data.loc[index, y_col_fitted_res_percent] \
                = (self.data.loc[index, y_col] - self.data.loc[index, y_col_fitted]) / self.data.loc[index, y_col]
            ...
            if label_fit:
                ...
                if label_fit_style["params_selection"] is None:
                    label_fit_style["params_selection"] = fit_label["beta0"]
                elif isinstance(label_fit_style["params_selection"], list):
                    print(f'Parameters not considered for fit label: '
                          f''''{", ".join([param for param in fit_label["beta0"]
                                                 if param not in label_fit_style["params_selection"]])
                                                }''') if debug else ""
                else:
                    raise Exception('label_fit_style["params_selection"] must be None or of type list')
                    ...
                    self._obj.loc[index, "label_fit"] += (
                        ...
                        + "\n    ".join(
                            [
                                ...
                                if label in label_fit_style["params_selection"]
                            ]
                        )
                        + "\n"
                    )
                ...
                # remove \n from end of label
                self._obj.loc[index, "label_fit"] = self._obj.loc[index, "label_fit"].strip("\n")

        if display_fit:
            if "label" not in self._obj.columns:  # initialize label
                self._obj.loc[:, "label"] = ""
            prev_labels = self._obj.loc[:, "label"].copy()  # deepcopy

            if label_fit_overwrite:  # reset label
                self._obj.loc[:, "label"] = ""
            ...
            if label_restore_prev:
                self._obj.loc[:, "label_fit"] = self._obj.loc[:, "label"]
                self._obj.loc[:, "label"] = prev_labels

        if ax_resplot is not None:
            self.plot(
                x_col=x_col,
                y_col=y_col_fitted_res_percent if ax_resplot_relative else y_col_fitted_res,
                data=self.data,
                ax=ax_resplot,
                label='',
                **plt_kwargs
    ...
    def export(
        ...
        **kwargs,
        ...
        if self.name == "":
            print(
                "\x1b[33m",
                "No file name given. Will be stored as 'Untitled'. ",
                "\x1b[0m",
            )
            self.name = 'Untitled'
        ...
        if db.MYSQL:
            from evaluation.utils.db_config import WHICH_JUPYTER
            jupyter_main_folder = 'jupyter' if WHICH_JUPYTER == 'prod' else 'jupyter_dev'
            path_relative_to_root = ""
            if os.name != "nt":
                path_relative_to_root = str(
                    Path(os.getcwd()).relative_to(
                        Path(os.getcwd()).parents[
                            os.getcwd().split("/").index(jupyter_main_folder) - 2
                        ]
                    )
                )
            ...
            if print_obsidian_link:
                unlinkable_symbols = ['#', '[', ']', '|', '^']
                if any([symbol in str(path_relative_to_root + str(plot_filename)) for symbol in unlinkable_symbols]):
                    print(
                        "\x1b[31m",
                        "Obsidian linking won't work if pathfile contains any of ",
                        str(unlinkable_symbols),
                        ".\n Please rename path/folder.",
                        "\x1b[0m",
                    )
                print(
                    "![["
...
# broken axis
def combine_broken_axes(ax_top_left,
                        ax_bottom_right,
                        broken_ratio=1,
                        broken_axis='y',
                        ):
    """
    Combine two axes into one axes with a broken axis. (Different dimension, with some part in between removed...)
    Will remove the splines in between and add diagonal lines
    Does not change:
        - axis lims
        -
    :param ax_top_left: matplotlib.Axes
        The ax in top (broken_axis='y') or on the left (broken_axis='x') side
    :param ax_bottom_right: matplotlib.Axes
        The ax in bottom (broken_axis='y') or on the right (broken_axis='x') side
    :param broken_ratio: float between 0 and 1
        size ratio of the plot in direction of broken_axis.
        You can calculate this from the axis grid by height_ratio/width_ratio
    :param broken_axis: str, one of ['y', 'x']
        which axis to break.
    :return: None
    """
    # hide the spines between ax1 and ax2
    hidden_spines = {'y': {'ax_top_left': 'bottom', 'ax_bottom_right': 'top'},
                     'x': {'ax_top_left': 'right', 'ax_bottom_right': 'left'}}
    # ticklabels = {'ax1': {'x': {}, 'y': {'>2000':2000, }}, 'ax2': {'x': {}, 'y':{}}}
    for ax, hidden_spine in zip([ax_top_left, ax_bottom_right],
                                hidden_spines[broken_axis].values(),
                                ):
        ax.spines[hidden_spine].set_visible(False)
    if broken_axis == 'y':
        ax_bottom_right.xaxis.tick_bottom()  # ticks on bottom
        ax_top_left.xaxis.tick_top()  # ticks on top
        ax_top_left.tick_params(labeltop=False)  # don't put tick labels at the top

        # ax_top_right.set_xticks([])
    elif broken_axis == 'x':
        # ax2.set_yticks([])
        ax_top_left.yaxis.tick_left()
        ax_top_left.tick_params(labelright=False)  # don't put tick labels at the right
        ax_bottom_right.yaxis.tick_right()

    d = {'x': 0.02, 'y': 0.015}[broken_axis]  # how big to make the diagonal lines in axes coordinates
    diagonal_coordinates = {
        'y': {'ax_top_right': [[(-d, +d), (-d * broken_ratio, +d * broken_ratio)],  # top-left diagonal
                               [(1 - d, 1 + d), (-d * broken_ratio, +d * broken_ratio)]
                               # top-right diagonal
                               ],
              'ax_bottom_left': [[(-d, +d), (1 - d, 1 + d)],
                                 [(1 - d, 1 + d), (1 - d, 1 + d)]
                                 ]
              },
        'x': {'ax_top_right': [[(1 - d, 1 + d), (1 - d, 1 + d)],  # top-right diagonal
                               [(1 - d, 1 + d), (-d, +d)]  # bottom-right diagonal
                               ],
              'ax_bottom_left': [[(-d / broken_ratio, +d / broken_ratio), (1 - d, 1 + d)],
                                 # top-left diagonal
                                 [(-d / broken_ratio, +d / broken_ratio), (-d, +d)]  # bottom-left diagonal
                                 ]
              },
        }
    for ax, diagonals in zip([ax_top_left, ax_bottom_right], diagonal_coordinates[broken_axis].values()):
        kwargs = dict(transform=ax.transAxes, color='k', clip_on=False)
        for diagonal in diagonals:
            ax.plot(diagonal[0], diagonal[1], **kwargs)
