...
def update_t_delay__s(
    ...
    :param names_isotope_analyte: optional, list of str
        list of names of analyte isotopes which should be displayed, if None all isotopes are displayed
    :param counts_log: bool, default False
        toggle whether to scale counts axis to log scale
    :return:
    """
    if type(id_exp_icpms) != int:
        raise Exception("id_exp_icpms must be a single integer")
    if names_isotope_analyte is None:
        names_isotope_analyte = []
    if type(names_isotope_analyte) == str:
        names_isotope_analyte = [names_isotope_analyte]
    ...
    exp_icpms = db.get_exp(
        ...
        + ("AND name_isotope_analyte IN (" + str(names_isotope_analyte).strip('[]')
            + ")" if len(names_isotope_analyte) > 0 else "")
        + ";",
    ...
    with plt.rc_context(
        ...
        exp_icpms = exp_icpms.dataset\
            .add_column('label', values=exp_icpms.index.get_level_values('name_isotope_analyte')) \
            .add_column('color', values='Accent') \
            .plot(
...
def update_counts_internalstandard_fitted(
    ...
    data_ec = db.get_data_ec(exp_ec)#, "data_ec_analysis")
    data_icpms = db.get_data_icpms(exp_icpms, match_ec_icpms,
                                    #con,
                                    ...
                                    t_start_shift__s=0,
                                    t_end_shift__s=0,
                                   apply_calibration_internalstandardfitting=False,
                                   # as internalstandardfitting is not available
    )
    data_icpms.loc[:, "count_ratio"] = (
        data_icpms.loc[:, "counts_analyte"]
        / data_icpms.loc[:, "counts_internalstandard"]
    )
    if 'counts_internalstandard_fitted' in data_icpms.columns:
    ...
    if display_calibration_limits:
        ...
        data_calibration = db.query_sql(
            ...
            + """);""",
            params=[int(val) for val in id_exp_icpms_calibrations],
            con=con,
            method="pandas",
            index_col=[
                "id_exp_icpms",
        ...
        exp_icpms_ML = (
            ...
            .fit(
                ...
                **{"color": color_ISTD_fit_complete_data}
                ...
                display_fit=True,
                add_data_to_export_object=True,
            )
            .return_dataset()
        )
...
def sfc_icpms_integration_analysis(
    ...
    icpms_restrict_baseline='before',
    ...
    :param ec_set_integrationlims: list of possible values
            ...
            'linked': set integration limits by selected dissolution timeframe
    :param icpms_set_baseline: list of possible values
            'fixed': baseline is set as 0
            'auto': baseline based on update_baseline_auto algorithm
            'manual': baseline points set by user manually
    :param icpms_restrict_baseline: default 'before', list of possible values
    ...
    if exp_ec.ec_name_technique.isin(["exp_ec_peis", "exp_ec_geis"]).any():
        ...
        if not data_eis.loc[:, ['Timestamp', 'E_dc__VvsRE', 'I_dc__A']].isna().any().any():
            # all EIS data time resolved available (eCat version >4.9)
            data_ec = pd.concat([data_ec, data_eis.rename(columns={'id_data_eis': 'id_data_ec',
                                                                   'E_dc__VvsRE': 'E_WE_raw__VvsRE',
                                                                   'I_dc__A': 'I__A'
                                                                   }
                                                          )]).sort_index()
        elif data_eis_avg_to_data_ec:
            # for eCat version <4.9 no time resolution available - averaging as optional work-around, not default
    ...
    with plt.rc_context(
        ...
        exp_ec = (
            ...
            .integrate(
                ...
                name_analysis_dropdown_useradded=name_analysis_dropdown_useradded,
            )
            .return_dataset()
        )
        ...
        exp_icpms = (
            ...
            .integrate(
                ...
                restrict_baseline=icpms_restrict_baseline,
...
def sfc_icpms_peakfitting(
    ...
    background_correction_constant_concentration=False,
    ...
    :param background_correction: optional, bool, Default True
        ...
        good fit results
    :param background_correction_constant_concentration: optional, bool, Default False
        substracts the minimum value as a constant from the dissolution data
    ...
    :param export_path: optional, str
        path where the plot should be stored. Default path depends on whether run on institute jupyterhub or in mybinder
        as defined in evaluation.utils.db_config
    :param name_isotope_analyte: str or None
        indices of icpms analyte isotopes, just test implemented
    :param name_isotope_internalstandard: str or None
        indices of icpms internalstandard isotopes, just test implemented
    ...
    exp_ec, data_ec, exp_icpms, data_icpms = db.get_exp_sfc_icpms(
        ...
        join_exp_ec_dataset_to_exp_ec='single',
    )

    if len(exp_icpms.index) == 0:
        print(
            "\x1b[31m",
            "No icpms experiment selected. Program stopped.",
            "\x1b[0m",
        )
    ...
    for index, row in exp_icpms.iterrows():
        ...
        if background_correction_constant_concentration:
            minvalue_dm_dt__ng_s = data_icpms_selected.loc[:, ('dm_dt__ng_s',)].min()
            data_icpms_selected['dm_dt__ng_s'] = data_icpms_selected['dm_dt__ng_s'] - float(minvalue_dm_dt__ng_s)
        ...
        # add value for additional baseline correction on df_ana_icpms_sfc_fitting-Dataframe
        if background_correction_constant_concentration:
            df_ana_icpms_sfc_fitting.loc[:, 'dm_dt_constant_background__ng_s'] = float(minvalue_dm_dt__ng_s)
        ...
        with plt.rc_context(
                ...
                for index_peak, row_peak in (
                    ...
                    if row_peak.fit_type == "background":
                        ...
                        if np.isnan(row_peak.mode_potential_error__VvsRHE) or np.isnan(
                            ...
                            signif_digits = Fit_SFC_ICP_MS_Dissolution.get_significant_digit(
                                row_peak.mode_potential_error__VvsRHE
                            )
                            labeltxt = (
                                    "Peak %s: " % row_peak.id_peak
                                    + "%s$\\pm$%s V" % (Fit_SFC_ICP_MS_Dissolution.ensure_significant_digit(
                                round(row_peak.mode_potential__VvsRHE,
                                      signif_digits),
                                signif_digits),
                                                        round(row_peak.mode_potential_error__VvsRHE,
                                                              signif_digits)
                                    )
                                    + "$_\\mathrm{RHE}$"
                            )
            ...
            def on_upload_button_clicked(changed_value):
                ...
                with engine.begin() as con:
                    ...
                    df_ana_icpms_sfc_fitting.loc[:, "t_inserted__timestamp"] = datetime.datetime.now()

                    plot_format = "svg"
                    plot_storage.name = "sfc_icpms_fit_" + "_".join(
                        [str(val) for val in df_ana_icpms_sfc_fitting.index[0]]
                    )
                    df_ana_icpms_sfc_fitting.loc[:, "file_path_plot_sfc_icpms_peakfit"] = str(export_path / (plot_storage.name + "." + plot_format))
