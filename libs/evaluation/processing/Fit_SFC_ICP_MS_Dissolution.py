...
def peak_detection_routine(
    ...
    for n, peak in enumerate(peak_idx):
        ...
        print("Starting Peak", n, "at ", time.iloc[peak], 's \t(id_data = ', peak, ')')
...
def get_significant_digit(err):
    ...
    ----------
    err : float
        Uncertainty that determines significance.
    ...
    """
    # err_str = str(err) # problem for floats displayed in scientific notation
    err_str = np.format_float_positional(err, trim='-')
