...
class IntegrateContainer:
    ...
    def __init__(
        ...
        restrict_baseline='before',
        set_integrationlims=None,  # #'auto', 'manual', 'nolims', 'linked'
        link_name_analysis=True,
        x_start_shift=0,
        x_end_shift=0,
        # multiple_exp=False,
        name_analysis_dropdown_useradded=None,
        **plt_kwargs,
    ):
        """
        ...
        :param set_baseline: list of possible values:
            ...
            'previous_selected': index of datapoint chosen as baseline is kept from previous analysis in tool
        :param restrict_baseline: default 'before', list of possible values:
            'before': point to derive baseline must be set earlier/before the beginning of integration
            'free': no restriction applied, baseline point can also be after the end point of integration
        :param set_integrationlims: None or str or list of possible values:
            'auto': integration limits are automatically set according to peak detection algorithm
            'manual': add sliders to adjust integration limits manually
            'nolims': integrate the whole experiment (not implemented)
            'linked': set integration limits by linked integration analysis
            'previous_selected': index of datapoints chosen as integration limit are kept from previous analysis in tool
        ...
        :param x_start_shift: int
            for linked name analysis, adjust window size (shift start of window) in which integration is possible
        :param x_end_shift: int
            for linked name analysis, adjust window size (shift end of window) in which integration is possible
        :param name_analysis_dropdown_useradded: list of str
            add additional entries to name_analysis dropdown
        :param plt_kwargs: keyword arguments of evaluation.visualization.plot
        """
        if name_analysis_dropdown_useradded is None:
            name_analysis_dropdown_useradded = []
        self.name_analysis_dropdown_useradded = name_analysis_dropdown_useradded

        if set_baseline is None:
            set_baseline = ["auto", "manual"]
        if set_integrationlims is None:
            set_integrationlims = ["auto", "manual"]
        if restrict_baseline not in ['before', 'free']:
            raise ValueError("restrict_baseline must be set as one of ['before', 'free']")
        else:
            self.restrict_baseline = restrict_baseline
        ...
        options_db = [option
                      for option
                      in self.df_integrate_names_analysis.name_analysis_init.dropna().unique().tolist()
                      if option not in self.name_analysis_dropdown_useradded]
        self.name_analysis_dropdown = widgets.Dropdown(
            options=[
                self.name_analysis_dropdown_choose,
                self.name_analysis_dropdown_new,
            ]
            + self.name_analysis_dropdown_useradded
            + options_db,
            # + [self.name_analysis_dropdown_new],
            # value='2',
        ...
        self.intSlider_integrate_begin = widgets.IntSlider(
            ...
            layout=Layout(
                width="500px",
                display="flex" if "manual" in self.set_integrationlims and "linked" not in self.set_integrationlims else "none",
            ),
            style={"description_width": "initial"},
        )
    ...
    def click_upload_button(self):
        ...
        with self.output_integration:
            ...
            # change export_name before update - to have correct file name of plot in database
            # (previously export_name was defined directly before figure export)
...
class Integrate:  # IntegrateContainer:
    ...
    def __init__(
        ...
        with plt.rc_context(self.integrate_container.plt_rcParams):
            ...
            # initialization of idx for slider values - dummy(?)
            self.idx_baseline = 0
            self.idx_integrate_begin = 0
            self.idx_integrate_min = 0
            self.idx_integrate_end = self.data_zoom.reset_index().index[-1]  # idx_integrate_end
        ...
        # dummy set left and right data to left and right edges
        ...
        self.linked_integration_lims = False
        self.link_integration_lims_to_parent = False
        if hasattr(self.integrate_container, 'parent_integrate_container'):
            if 'linked' in self.integrate_container.parent_integrate_container.set_integrationlims:
                self.link_integration_lims_to_parent = True
    ...
    def activate(self, overview=False):
        ...
        if not overview:
            ...
            # Initialization of integration limits,
            ...
            # Initialization of integration values to default,  will be overwritten by database/auto/prev_selected
            self.idx_baseline = self.idx_integrate_min
            self.idx_integrate_begin = self.idx_integrate_min
            self.idx_integrate_end = self.idx_integrate_max

            with self.integrate_container.output_integration:
                print("\x1b[35m", 'Integration parameters initialized to start/end of integration limits', "\x1b[0m")
            ...
            if self.id_ana_integration is not None:
                with self.integrate_container.output_integration:
                    ...
                    if self.auto_integration:
                        print("\x1b[35m" + " Integration parameter derived from auto-algorithm." + "\x1b[0m")
                    else:
                        print("\x1b[35m" + " Integration parameters were manually adjusted." + "\x1b[0m")
            elif "auto" in self.integrate_container.set_baseline:
                with self.integrate_container.output_integration:
                    clear_output()
                    print("\x1b[35m", ' Derive integration parameter from auto-algorithm', "\x1b[0m")
                self.update_baseline()


            elif "prev_selected" in self.integrate_container.set_baseline\
                    or "prev_selected" in self.integrate_container.set_integrationlims:
                self.prev_selected_vals = []

                def keep_prev_selected(prev_value, self_idx, name, in_intlims=True):
                    if (prev_value != 0
                            and (self.idx_integrate_min <= prev_value <= self.idx_integrate_max or not in_intlims)):
                        self.prev_selected_vals += [name]
                        return prev_value
                    else:
                        return self_idx

                if "prev_selected" in self.integrate_container.set_baseline:
                    self.no_of_datapoints_avg = keep_prev_selected(
                        self.integrate_container.intText_no_of_datapoints_avg.value,
                        self.no_of_datapoints_avg,
                        'no_of_datapoints_avg',
                        in_intlims=False
                    )
                    self.idx_baseline = keep_prev_selected(
                        self.integrate_container.intSlider_baseline.value,
                        self.idx_baseline,
                        'idx_baseline'
                    )
                if "prev_selected" in self.integrate_container.set_integrationlims:
                    self.idx_integrate_begin = keep_prev_selected(
                        self.integrate_container.intSlider_integrate_begin.value,
                        self.idx_integrate_begin,
                        'idx_integrate_begin'
                    )
                    self.idx_integrate_end = keep_prev_selected(
                        self.integrate_container.intSlider_integrate_end.value,
                        self.idx_integrate_end,
                        'idx_integrate_end'
                    )
                with self.integrate_container.output_integration:
                    if len(self.prev_selected_vals) == 4:
                        clear_output()
                    if len(self.prev_selected_vals) >= 1:
                        print(
                            "\x1b[35m"
                            + ' Values kept from previous selection for: '
                            + ', '.join([name for name in self.prev_selected_vals])
                            + "\x1b[0m"
                        )

            # update integration limits of analysis linked to this, if exists
            self.update_link_integration_lims_to_parent(which='both')

            # first limits of intSlider_baseline need to be set than intText_no_of_datapoints_rolling van initiated

            # Setting slider widgets minima/maxima
            # display(self.data_zoom)
            ...
            ) if self.integrate_container.restrict_baseline == 'before' else self.idx_integrate_min# self.data_left.index.min()
            ...
            self.update_integration_data(print_message=False)  # intText_no_of_datapoints_avg.value)
            self.update_integration_plot()
    ...
    def to_database(self, con):
        ...
        print(self.integrate_container.export_path, self.integrate_container.export_object.name, datetime.datetime.now())
        ...
        df_ana_integration = pd.DataFrame(
            {
                ...
                "t_updated_integration__timestamp": datetime.datetime.now(),
                "file_path_plot_integration": str(Path(self.integrate_container.export_path)/
                                                  self.integrate_container.export_object.name)+'.svg',
        ...
        if self.id_ana_integration is None:
            ...
            self.integrate_container.df_integrate_names_analysis.loc[
                ((self.integrate_container.df_integrate_names_analysis.index.isin(
                    self.exp_index if len(self.exp_index) == 1 else [self.exp_index, ]
                    ))
                 & (self.integrate_container.df_integrate_names_analysis.name_analysis_init
                    == self.integrate_container.name_analysis_dropdown.value)),
                "name_analysis_init"
            ] = self.integrate_container.name_analysis_text.value
    ...
    def set_idx_baseline(self, value, update):
        ...
        if self.integrate_container.restrict_baseline == 'before':
            self.integrate_container.intSlider_integrate_begin.min = self.idx_baseline
        ...
        if update:
            with self.integrate_container.output_integration:
                clear_output()
            self.auto_integration = False
            # update idx_integrate_begin (just for purpose of error message) of parent analysis if exists
            self.update_link_integration_lims_to_parent(which='')
    ...
    def set_idx_integrate_begin(self, value, update):
        ...
        if update:
            with self.integrate_container.output_integration:
                clear_output()
            self.auto_integration = False
            # update idx_integrate_begin of parent analysis if exists
            self.update_link_integration_lims_to_parent(which='begin')
    ...
    def set_idx_integrate_end(self, value, update):
        ...
        if update:
            with self.integrate_container.output_integration:
                clear_output()
            self.auto_integration = False
            # update idx_integrate_end of parent analysis if exists
            self.update_link_integration_lims_to_parent(which='end')

            self.update_integration_data()  # intText_no_of_datapoints_avg.value)
            self.update_integration_plot()
            self.widget_fires_update = True

    def update_link_integration_lims_to_parent(self, which='both'):

        def find_closest(idx_integrate):
            xval_integrate = self.data.loc[:, self.integrate_container.x_col].iloc[idx_integrate]
            xval_integrate_differences_linked \
                = (parent_integrate.data.loc[:, parent_integrate.integrate_container.x_col]
                    - xval_integrate
                   ).abs()
            xval_integrate_differences_linked_min = xval_integrate_differences_linked.min()
            idx_integrate_linked = xval_integrate_differences_linked.idxmin()[-1]
            return xval_integrate, xval_integrate_differences_linked_min, idx_integrate_linked

        if self.link_integration_lims_to_parent:
            parent_integrate = self.integrate_container.parent_integrate_container.active()
            parent_integrate.linked_integration_lims = True

            xval_integrate_begin, xval_integrate_begin_differences_linked_min, idx_integrate_linked_begin \
                = find_closest(self.idx_integrate_begin)
            xval_integrate_end, xval_integrate_end_differences_linked_min, idx_integrate_linked_end \
                = find_closest(self.idx_integrate_end)

            window_length = xval_integrate_end - xval_integrate_begin

            # both limits are always recalculated, otherwise differences would need to be stored to always
            # print deviation error message
            for (xval_integrate_differences_linked_min,
                 name) in zip([xval_integrate_begin_differences_linked_min, xval_integrate_end_differences_linked_min],
                              ['begin', 'end']
                              ):
                with self.integrate_container.output_integration:
                    # print in  child not parent analysis - there the manual change is made (better visibility)
                    color = '33'  # warning
                    if xval_integrate_differences_linked_min > 0.01 * window_length:
                        print("\x1b[31m"
                              + "Bad matching of derived " + name
                              + " integration from linked analysis:"
                              + "\x1b[0m")
                        color = '31'  # error
                    print("\x1b[%sm" % color
                          + "   Discrepancy in x-" + name
                          + " from linked analysis = " + '%.2f' % xval_integrate_differences_linked_min
                          + ' = ' + '%.0f' % (xval_integrate_differences_linked_min / window_length * 100)
                          + '% of window length'
                          + "\x1b[0m")

            if which in ['both', 'end']:
                self.integrate_container.parent_integrate_container.intSlider_integrate_end.value \
                    = idx_integrate_linked_end
            if which in ['both', 'begin']:
                self.integrate_container.parent_integrate_container.intSlider_integrate_begin.value \
                    = idx_integrate_linked_begin
    ...
    def update_integration_data(self, print_message=True):
        with self.integrate_container.output_integration:
            if print_message:
                if self.linked_integration_lims:
                    print(
                        "\x1b[35m" + "Integration limits adjusted according to linked analysis" + "\x1b[0m"
                    )
                #elif self.auto_integration:
                #    print(
                #        "\x1b[35m"
            ...
            xval_integrate_begin = self.data.loc[:, self.integrate_container.x_col].iloc[
                    self.idx_integrate_begin
                ]
            xval_integrate_end = self.data.loc[:, self.integrate_container.x_col].iloc[
                    self.idx_integrate_end
                ]
            xval_unit = self.integrate_container.x_col.split("__")[-1]
            print(
                "integration limits: ",
                xval_integrate_begin,
                xval_unit,
                xval_integrate_end,
                xval_unit,
                '; window length:',
                xval_integrate_end-xval_integrate_begin,
                xval_unit
            )
