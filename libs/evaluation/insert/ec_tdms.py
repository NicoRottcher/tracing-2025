...
def read_tdms(file_path):
    ...
    with TdmsFile.open(file_path) as tdms_file:
        ...
        for group in tdms_file.groups():
            ...
            # fail with spaces: for eCat version at least <4.13 there is a space between technique and number
            df.loc[:, "data_ec_tablename"] = df.loc[:, "data_ec_tablename"].str.replace(' ', '')
            ...
            # Technique lines
                ...
                # elif technique_dict['Technique'] == 'Gases': # and other handlers
                    ...
                    exp_ec_dict["data_ec_tablename"] = (
                        ...
                        + "_"
                          # space fail: handled in read tdms
                          #( if (int(exp_ec_dict["labview_sfc_version"][0].split('.')[0]) >= 5
                          #       or (int(exp_ec_dict["labview_sfc_version"][0].split('.')[0]) == 4
                          #           and int(exp_ec_dict["labview_sfc_version"][0].split('.')[1]) >= 13)
                          #       ) else "_ ")
                          # at least since eCat version 5.0 no space between technique and number,
                          # 20240114: also for eCat version 4.13 no space between technique and number,
                        + technique_dict["id_ML_technique"]
                    )
                    # Just to make sure remove also spaces here as in the tdms file, in case there are other spaces
                    exp_ec_dict["data_ec_tablename"] = exp_ec_dict["data_ec_tablename"].replace(' ', '')
...
def insert_ec_tdms(path_to_files, manual_info):
    ...
    :return:
    """
    print("You are using Version 20240820.")
        ...
        if tdms_file.is_file():
            ...
            if not info_file.is_file():
                ...
                with engine.begin() as conn:  # new connection once all files are read
                    ...
                    if any([a != b
                            for a, b
                            in zip(exp_ec_list.loc[:, "id_ML_technique"].to_numpy(),
                                   exp_ec_list.sort_values(by=["id_ML_technique"]).reset_index()
                                              .loc[:, "id_ML_technique"].to_numpy()
                                   )]
                           ):
                        warnings.warn(
